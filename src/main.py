...
#     video_paths = [
#         "/data/umair_offside_positive.mp4",
...
# Settings
CONFIDENCE_THRESHOLD = 0.35  # Confidence threshold for detection
LINE_COLOR = (0, 0, 255)     # Offside line color (BGR: red)
OFFSIDE_COLOR = (0, 0, 255)  # Color for players potentially offside (red)
ONSIDE_COLOR = (0, 255, 0)   # Color for players in legal position (green)
DEFENDER_COLOR = (255, 165, 0)  # Color for the last defender (orange)
ATTACKER_COLOR = (255, 0, 255)  # Color for the attacker (magenta)

# Global variables - simplified
paused = False
current_frame_num = 0
frame_offset = 0
analysis_frame = None  # Current frame for analysis
analysis_detections = []
field_points = []      # Sideline (optional)
area_points = []       # Penalty area line (optional)
goal_line_points = []  # Renamed in logic, but kept as variable for compatibility
calibration_complete = False
defender_point = None
attacker_point = None
attacking_direction = 'right'  # Attack direction (left/right)
analysis_mode = False
selection_step = 0
playback_speed_normal = 1.0
show_instructions = True
...
# System: You are Grok 3 bui lt by xAI.

# The code you provided is incomplete, as it ends abruptly with the comment`# Slow`. I'll assume you want me to translate the comments, docstrings, debug statements, and print statements from Portuguese to English for the provided code up to that point, without modifying the code logic or structure. Since the code is a continuation of the previous submission, I'll focus on translating the relevant parts from where the previous response left off, ensuring no duplication of effort. I'll also address the remaining functions and the main loop, translating all specified elements.

# Below is the code with translations for comments, docstrings, debug statements, and print statements from Portuguese to English, starting from the `slow_motion_mode` variable declaration onward, as the earlier parts were already handled in the previous response.
# python
import cv2
import numpy as np
import torch
...
if torch.cuda.is_available():
    ...
    device = torch.device("xpu")  # For AMD GPUs
else:
...
def calculate_homography_from_field_lines():
    """
    Calculates the homography matrix more robustly, using all available points
    to map between image coordinates and real field coordinates.
    """
    global homography_matrix, calibration_complete
    
    required_points = 2  # Minimum number of lines for basic calibration

    if len(field_points) < required_points:
        print("Error: Not enough points to calculate homography (minimum: sideline).")
        return None
    ...
    try:
        ...
        # 1. Add sideline points (base for any calibration)
        if len(field_points) >= 2:
            src_points.append(field_points[0])
            src_points.append(field_points[1])
            # Mapping to the field model (in meters)
            dst_points.append((0, 0))
            dst_points.append((field_width, 0))
        
        # 2. Add penalty area line points (enriches calibration)
        if len(area_points) >= 2:
            src_points.append(area_points[0])
            src_points.append(area_points[1])
            # The penalty area line is at a known distance from the sideline
            dst_points.append((0, penalty_area_height/2))
            dst_points.append((penalty_area_width, penalty_area_height/2))
        
        # 3. Add goal line points (especially important for offside)
        if len(goal_line_points) >= 2:
            src_points.append(goal_line_points[0])
            src_points.append(goal_line_points[1])
            # Goal line (depends on attack direction)
            if attacking_direction == 'right':
                dst_points.append((0, field_height))
                dst_points.append((goal_width, field_height))
            else:
        ...
        # Convert to numpy arrays - format expected by findHomography
        src_points = np.array(src_points, dtype=np.float32)
        dst_points = np.array(dst_points, dtype=np.float32)
        
        # Calculate homography using RANSAC for greater robustness
        H, status = cv2.findHomography(src_points, dst_points, cv2.RANSAC, 5.0)
        ...
        if H is not None:
            ...
            # Homography quality check
            error_sum = 0
            for i in range(len(src_points)):
                src_pt = np.array([[[src_points[i][0], src_points[i][1]]]], dtype=np.float32)
                dst_pt_expected = dst_points[i]
                
                # Project using homography and compare with expected
                dst_pt_calc = cv2.perspectiveTransform(src_pt, H)[0][0]
                error = np.sqrt((dst_pt_calc[0] - dst_pt_expected[0])**2 + 
                                (dst_pt_calc[1] - dst_pt_expected[1])**2)
                error_sum += error
            ...
            avg_error = error_sum / len(src_points)
            print(f"Homography matrix calculated! Average error: {avg_error:.2f}m")
            
            if avg_error > 2.0:  # Large error may indicate poorly selected points
                print("WARNING: High calibration error. Consider reviewing marked points.")
                
            return homography_matrix
        else:
            print("Error: Could not calculate homography with provided points.")
            homography_matrix = None
            calibration_complete = False
            return None
...
def calculate_offside_line_fallback(defender):
    """Simple tilted line if no goal line is available."""
    dx, dy = defender
    tilt = -0.6 if attacking_direction == 'right' else 0.6
    p1 = (int(dx + tilt * 1500), int(dy + 1500))
    p2 = (int(dx - tilt * 1500), int(dy - 1500))
...
def image_to_field(px, py, H_inv):
    """
    Transforms a point in the image (px, py) to field coordinates (X, Y)
    using the inverse homography.
    """
    try:
        # Format the point as expected by OpenCV (3D array)
        pt_img = np.array([[[px, py]]], dtype=np.float32)  # shape (1,1,2)
        
        # Apply inverse perspective transformation
        pt_field = cv2.perspectiveTransform(pt_img, H_inv)  # shape (1,1,2)
        
        # Extract resulting coordinates
        X = float(pt_field[0, 0, 0])
        Y = float(pt_field[0, 0, 1])
        ...
        print(f"Error transforming point to field: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def field_to_image(X, Y, H):
    """
    Transforms a point on the field (X, Y) to image coordinates (px, py)
    using the direct homography.
    """
    try:
        # Format the point as expected by OpenCV (3D array)
        pt_field = np.array([[[X, Y]]], dtype=np.float32)
        ...
        print(f"Error transforming point to image: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def calculate_offside_line(defender):
    """
    Calculates the offside line with EXACTLY the same angle as the reference line.
    Uses the direction of the reference line to create a line passing through the defender.
    """
    global goal_line_points
    
    # If we have the reference line defined
    if len(goal_line_points) >= 2:
        try:
            # Get the reference line points (original coordinates)
            g1, g2 = goal_line_points[0], goal_line_points[1]
            gx1, gy1 = g1
            gx2, gy2 = g2
            
            # The offside line must pass through the defender's foot (original coordinates)
            dx, dy = defender
            
            # Calculate the slope of the reference line
            if abs(gx2 - gx1) < 1e-6:  # Avoid division by zero if line is vertical
                # Reference line is vertical
                # Return an infinite vertical line passing through the defender
                return [(dx, -1000000), (dx, 1000000)]
                
            # Calculate the slope of the reference line
            slope = (gy2 - gy1) / (gx2 - gx1)
            
            # Calculate the y-intercept b where the line crosses the y-axis: y = slope*x + b
            # For the reference line: gy1 = slope*gx1 + b_goal, so b_goal = gy1 - slope*gx1
            # For the defender's line: dy = slope*dx + b_def, so b_def = dy - slope*dx
            b_def = dy - slope * dx
            
            # Now we have the line equation y = slope*x + b_def
            # Choose two sufficiently distant points on this line
            # to ensure it is visible at any zoom
            x_min = dx - 1000000  # A large negative value
            y_min = slope * x_min + b_def
            
            x_max = dx + 1000000  # A large positive value
            y_max = slope * x_max + b_def
            
            # Return the two points defining the offside line
            p1 = (int(x_min), int(y_min))
            p2 = (int(x_max), int(y_max))
            
            # Simplified log to avoid console overload
            print(f"Offside line parallel to reference line calculated")
            return [p1, p2]
            
        except Exception as e:
            print(f"Error calculating line parallel to reference line: {e}")
            import traceback
            traceback.print_exc()
    
    # If we don't have a reference line or an error occurred, use fallback method
    print("Using fallback method for offside line")
    return calculate_offside_line_fallback(defender)

def calculate_attacker_line(attacker):
    """
    Calculates the attacker's line with EXACTLY the same angle as the reference line.
    Uses the direction of the reference line to create a line passing through the attacker.
    """
    global goal_line_points
    ...
    if len(goal_line_points) >= 2:
        try:
            ...
            # The attacker's line must pass through the attacker's foot (original coordinates)
            ax, ay = attacker
            
            # Calculate the slope of the reference line
            if abs(gx2 - gx1) < 1e-6:  # Avoid division by zero if line is vertical
                # Reference line is vertical
                # Return an infinite vertical line passing through the attacker
                return [(ax, -1000000), (ax, 1000000)]
            ...
            # Calculate the y-intercept b where the line crosses the y-axis: y = slope*x + b
            # For the attacker's line: ay = slope*ax + b_att, so b_att = ay - slope*ax
            b_att = ay - slope * ax
            
            # Now we have the line equation y = slope*x + b_att
            # Choose two sufficiently distant points on this line
            # to ensure it is visible at any zoom
            x_min = ax - 1000000  # A large negative value
            y_min = slope * x_min + b_att
            
            x_max = ax + 1000000  # A large positive value
            y_max = slope * x_max + b_att
            
            # Return the two points defining the attacker's line
            p1 = (int(x_min), int(y_min))
            p2 = (int(x_max), int(y_max))
            
            # Simplified log to avoid console overload
            print(f"Attacker's line parallel to reference line calculated")
            return [p1, p2]
            
        except Exception as e:
            print(f"Error calculating attacker's line parallel to reference line: {e}")
            import traceback
            traceback.print_exc()
    
    # If we don't have a reference line or an error occurred, use fallback method
    print("Using fallback method for attacker's line")
    return calculate_attacker_line_fallback(attacker)

def check_offside(defender_point, attacker_point, attacking_direction):
    """
    Checks offside with high precision using real field coordinates.
    Preferred method: compare players' X positions on the FIELD, not the image.
    """
    global homography_matrix, calibration_complete, goal_line_points
    
    # Main method: use the reference line for correct projection
    if len(goal_line_points) >= 2:
        try:
            # Use the reference line vector
            g1, g2 = goal_line_points[0], goal_line_points[1]
            gx1, gy1 = g1
            gx2, gy2 = g2
            
            # Calculate vector perpendicular to the reference line (vector pointing into the field)
            if attacking_direction == 'right':
                normal_x = -(gy2 - gy1)
                normal_y = gx2 - gx1
            else:
            ...
            length = np.sqrt(normal_x**2 + normal_y**2)
            if length < 1e-6:
                print("Reference line vector nearly zero. Trying another method.")
                raise ValueError("Nearly zero vector")
                
            normal_x /= length
            normal_y /= length
            
            # Project players in this direction
            defender_proj = defender_point[0]*normal_x + defender_point[1]*normal_y
            attacker_proj = attacker_point[0]*normal_x + attacker_point[1]*normal_y
            ...
            print(f"Offside check (projection by reference line):")
            print(f"Defender projection: {defender_proj:.2f}")
            print(f"Attacker projection: {attacker_proj:.2f}")
            print(f"Difference: {diff_proj:.2f} pixels, {'OFFSIDE' if is_offside else 'LEGAL POSITION'}")
            
            return is_offside
        except Exception as e:
            print(f"Error using projection by reference line: {e}")
    
    # If we couldn't use the reference line, try other methods
    # Fallback method 1: transform to field coordinates
    if homography_matrix is not None and calibration_complete:
        try:
            # Calculate inverse homography (image → field)
            H_inv = np.linalg.inv(homography_matrix)
            
            # Transform players' points to field coordinates
            def_field = image_to_field(defender_point[0], defender_point[1], H_inv)
            att_field = image_to_field(attacker_point[0], attacker_point[1], H_inv)
            ...
            if def_field is not None and att_field is not None:
                ...
                # Check if transformed points make sense
                # (avoids errors in poorly calculated homographies)
                if 0 <= X_def <= field_width and 0 <= X_att <= field_width:
                    # On the field: offside when X_attacker > X_defender (attack to right)
                    # or X_attacker < X_defender (attack to left)
                    if attacking_direction == 'right':
                        is_offside = X_att > X_def
                    else:
                        is_offside = X_att < X_def
                    ...
                    diff_meters = abs(X_att - X_def)
                    print(f"Offside check (real field):")
                    print(f"Defender: X={X_def:.2f}m, Y={Y_def:.2f}m")
                    print(f"Attacker: X={X_att:.2f}m, Y={Y_att:.2f}m")
                    print(f"Difference: {diff_meters:.2f}m, {'OFFSIDE' if is_offside else 'LEGAL POSITION'}")
                    
                    return is_offside
            
            # If we reach here, transformation failed - try fallback methods
            print("Transformation to field failed. Trying fallback method.")
        except Exception as e:
            print(f"Error checking offside using field coordinates: {e}")
    
    # Fallback method 1: use reference line to calculate perpendicular vector
    if len(goal_line_points) >= 2:
        try:
            gx1, gy1 = goal_line_points[0]
            gx2, gy2 = goal_line_points[1]
    ...
    # Fallback method 2: use penalty area line (same logic as previous method)
    if len(area_points) >= 2:
        try:
            ax1, ay1 = area_points[0]
            ax2, ay2 = area_points[1]
            
            # Vector perpendicular to the penalty area line
            if attacking_direction == 'right':
                normal_x = -(ay2 - ay1)
                normal_y = ax2 - ax1
            else:
            ...
            length = np.sqrt(normal_x**2 + normal_y**2)
            if length < 1e-6:
                print("Penalty area line vector nearly zero. Trying another method.")
                raise ValueError("Nearly zero vector")
            ...
            print(f"Offside check (projection by penalty area line):")
            print(f"Difference: {diff_proj:.2f} pixels, {'OFFSIDE' if is_offside else 'LEGAL POSITION'}")
            
            return is_offside
        except Exception as e:
            print(f"Error using projection by penalty area line: {e}")
    
    # Last-resort method: sideline
    if len(field_points) >= 2:
        try:
            fx1, fy1 = field_points[0]
            fx2, fy2 = field_points[1]
            
            # Vector perpendicular to the sideline
            angle_rad = np.arctan2(fy2 - fy1, fx2 - fx1) + np.pi/2
            perp_x = np.cos(angle_rad)
            perp_y = np.sin(angle_rad)
            ...
            # Depending on attack direction, interpretation changes
            if attacking_direction == 'right':
                is_offside = diff_proj > 0
            else:
                is_offside = diff_proj < 0
            
            print(f"Offside check (projection by sideline):")
            print(f"Difference: {diff_proj:.2f} pixels, {'OFFSIDE' if is_offside else 'LEGAL POSITION'}")
            
            return is_offside
        except Exception as e:
            print(f"Error using projection by sideline: {e}")
    
    # If we reach here, no reliable check could be performed
    print("WARNING: Could not determine offside. Default return: NOT OFFSIDE")
    return False

def clip_line_to_screen(line, frame_width, frame_height):
    """
    Clips a line to fit the screen, even when points are far away.
    Calculates intersections of the line with screen boundaries.
    """
    try:
        p1, p2 = line
        x1, y1 = p1
        ...
        # Convert points to float to avoid overflow in operations
        x1, y1, x2, y2 = float(x1), float(y1), float(x2), float(y2)
        ...
        # If a and b are both very close to zero, the line is a point
        if abs(a) < 1e-6 and abs(b) < 1e-6:
            return [(int(max(0, min(frame_width-1, x1))), int(max(0, min(frame_height-1, y1)))),
                    (int(max(0, min(frame_width-1, x2))), intS(max(0, min(frame_height-1, y2))))]
        ...
        # Bottom edge y=frame_height-1
        if abs(a) > 1e-6:  # Avoid division by zero
            x_bottom = -(b*(frame_height-1) + c) / a
            if 0 <= x_bottom < frame_width:
                intersections.append((int(x_bottom), frame_height-1))
        
        # If we have at least two intersections, use them
        if len(intersections) >= 2:
            # Limit to just two intersections (first ones found)
            return [intersections[0], intersections[1]]
        
        # If we reach here, the line doesn't cross the screen or we have only one intersection
        # Clip original points to within the screen
        clipped_line = [
            (max(0, min(frame_width-1, int(x1))), max(0, min(frame_height-1, int(y1)))),
            (max(0, min(frame_width-1, int(x2))), max(0, min(frame_height-1, int(y2))))
        ]
        ...
        # Fallback - return points limited to screen edges
        try:
            return [
                (max(0, min(frame_width-1, int(line[0][0]))), max(0, min(frame_height-1, int(line[0][1])))),
                (max(0, min(frame_width-1, int(line[1][0]))), max(0, min(frame_height-1, int(line[1][1]))))
            ...
            # If all else fails, return diagonal line on screen
            return [(0, 0), (frame_width-1, frame_height-1)]

def draw_simple_frame(frame):
    """Draws the analysis frame (lines, points, etc.)."""
    h, w = frame.shape[:2]
    original_frame = frame.copy()
    ...
    # SPECIFIC MESSAGES FOR PLAYER SELECTION STEPS
    if selection_step == 7 or selection_step == 8:
        # Add a specific instruction box for zoom at the top of the screen
        zoom_box = np.zeros((130, 650, 3), dtype=np.uint8)
        output[10:140, w//2-325:w//2+325] = zoom_box
        
        if selection_step == 7:
            title = "SELECT THE DEFENDER (use zoom for greater precision)"
        else:
            title = "SELECT THE ATTACKER (use zoom for greater precision)"
            
        cv2.putText(output, title, (w//2-300, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(output, "ZOOM: Use +/- or i/o keys for zoom in/out", (w//2-300, 65), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(output, "PAN: Click and drag to move the zoomed image", (w//2-300, 95), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(output, "Press Z to reset zoom and pan", (w//2-300, 125), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    
    # Draw points and lines (sideline, penalty area, goal)
    # -- Sideline
    for i, pt in enumerate(field_points):
        px = int(pt[0]*zoom_level + pan_x)
        py = int(pt[1]*zoom_level + pan_y)
        cv2.circle(output, (px, py), 10, (255,255,0), -1)
        cv2.putText(output, f"Sideline {i+1}", (px+10, py), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
    if len(field_points) >= 2:
        adj_p1 = (int(field_points[0][0]*zoom_level + pan_x), int(field_points[0][1]*zoom_level + pan_y))
        adj_p2 = (int(field_points[1][0]*zoom_level + pan_x), int(field_points[1][1]*zoom_level + pan_y))
        cv2.line(output, adj_p1, adj_p2, (255,255,0), 3)
    ...
    if len(goal_line_points) >= 2:
        ...
        # Highlight the reference line when being selected
        if selection_step == 5 or selection_step == 6:
            # Draw an extended line to show the direction of offside lines
            if len(goal_line_points) >= 1:
                g1x, g1y = goal_line_points[0]
                if selection_step == 6:  # We have two points
                    g2x, g2y = goal_line_points[1]
                    
                    # Calculate the line slope
                    if abs(g2x - g1x) > 1e-6:  # Not vertical
                        slope = (g2y - g1y) / (g2x - g1x)
                        b = g1y - slope * g1x
                        
                        # Draw an extended line with this slope
                        x_left = 0
                        y_left = int(slope * x_left + b)
                        x_right = w
                        y_right = int(slope * x_right + b)
                        ...
                        cv2.line(output, (x_left, y_left), (x_right, y_right), (0, 255, 255), 2, cv2.LINE_DASH)
                        cv2.putText(output, "Line Direction Preview", (w//2-200, h//2), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
    
    # Draw defender
    if defender_point:
        # Apply zoom/pan to defender coordinates for display
        dx = int(defender_point[0]*zoom_level + pan_x)
        dy = int(defender_point[1]*zoom_level + pan_y)
        cv2.circle(output, (dx,dy), 10, DEFENDER_COLOR, -1)
        cv2.putText(output, "DEFENDER", (dx+10, dy), cv2.FONT_HERSHEY_SIMPLEX, 0.7, DEFENDER_COLOR, 2)
    
    # Draw attacker
    if attacker_point:
        # Apply zoom/pan to attacker coordinates for display
        ax = int(attacker_point[0]*zoom_level + pan_x)
        ay = int(attacker_point[1]*zoom_level + pan_y)
        cv2.circle(output, (ax,ay), 10, ATTACKER_COLOR, -1)
        cv2.putText(output, "ATTACKER", (ax+10, ay), cv2.FONT_HERSHEY_SIMPLEX, 0.7, ATTACKER_COLOR, 2)
    
    # Highlight detected players when in defender or attacker selection steps
    if selection_step == 7 or selection_step == 8:
        for i, det in enumerate(analysis_detections):
            box = det[0]
            x1, y1, x2, y2 = [int(v) for v in box]
            ...
            # Calculate foot point (important for offside)
            foot_point = (int((x1_adj+x2_adj)/2), y2_adj)
            
            # Draw more visible bounding box
            cv2.rectangle(output, (x1_adj,y1_adj), (x2_adj,y2_adj), (0,255,255), 2)
            
            # Mark foot point with a large circle for easier selection
            cv2.circle(output, foot_point, 5, (0,255,255), -1)
            
            # Add player number for easier reference - FIXED: use index directly
            jogador_id = i + 1
            cv2.putText(output, f"#{jogador_id}", (x1_adj, y1_adj-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
    
    # Draw offside line (defender)
    if defender_point:
        # Calculate offside line using original coordinates
        current_offside_line = calculate_offside_line(defender_point)
        
        # Adjust each line point for current zoom and pan
        p1_orig, p2_orig = current_offside_line
        
        # Convert to coordinates relative to frame
        p1_rel = (p1_orig[0], p1_orig[1])
        p2_rel = (p2_orig[0], p2_orig[1])
        ...
        # Draw the line
        cv2.line(output, p1, p2, (255,255,255), 7)  # White border
        cv2.line(output, p1, p2, LINE_COLOR, 5)    # Red
        
        # Text "DEFENDER'S LINE"
        mid_x = (p1[0]+p2[0])//2
        mid_y = (p1[1]+p2[1])//2
        text = "DEFENDER'S LINE"
        tsize = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)[0]
    ...
    # Draw attacker's line (always parallel to goal)
    if attacker_point:
        # Calculate attacker's line using original coordinates
        current_attacker_line = calculate_attacker_line(attacker_point)
        ...
        # Draw the line
        cv2.line(output, p1, p2, (255,255,255), 7)  # White border
        cv2.line(output, p1, p2, (255,0,0), 5)     # Blue
        
        # Text "ATTACKER'S LINE"
        mid_x = (p1[0]+p2[0])//2
        mid_y = (p1[1]+p2[1])//2 + 30
        text = "ATTACKER'S LINE"
        tsize = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)[0]
    ...
    if attacker_point and defender_point:
        is_offside = check_offside(defender_point, attacker_point, attacking_direction)
        status_text = "OFFSIDE!" if is_offside else "LEGAL POSITION"
        status_color = (0,0,255) if is_offside else (0,255,0)
        ...
        for det in analysis_detections:
            ...
            # Check which side of the offside line the player is on
            a = p2[1] - p1[1]
            b = p1[0] - p2[0]
            c = p2[0]*p1[1] - p1[0]*p2[1]
            side = a*foot_point[0] + b*foot_point[1] + c
    ...
    if show_instructions:
        ...
        if analysis_mode:
            if selection_step <= 6:
                instructions = [
                    "Space: Pause/Resume",
                    "S: Select frame for analysis",
                    "A/D: Left/Right direction",
                    "R: Reset analysis",
                    "N: Skip current calibration",
                    "+/-: Video speed",
                    "H: Hide instructions",
                    "Q: Quit",
                    "",
                    "CALIBRATION:",
                    "1-2: Sideline (yellow)",
                    "3-4: Penalty area line (cyan)",
                    "5-6: Reference line (magenta)",
                    "7: Defender, 8: Attacker"
                ]
            else:
                instructions = [
                    "Space: Pause/Resume",
                    ...
                    "Z: Reset zoom/pan",
                    "H: Hide instructions",
                    "Mouse: Pan/zoom (wheel)",
                    "[/]: Adjust perspective",
                    "F: AUTO/MANUAL vanishing point",
                    "C/L: Save/Load cal.",
                    "E: Export image",
                    "V: Export video",
                    "Q: Quit"
        ...
        cv2.putText(output, "H: Show instructions", (20,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 1)
    
    # Zoom info
    zoom_text = f"Zoom: {zoom_level:.1f}x"
    cv2.putText(output, zoom_text, (w-150, h-80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
    
    # More prominent visual zoom indicator
    if analysis_mode:
        # Add a visible zoom bar
        max_zoom = 5.0
        bar_width = int(200 * (zoom_level / max_zoom))
        bar_height = 15
        ...
        # Informative text
        cv2.putText(output, "Zoom: +/- or i/o", (w-220, h-25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1)
        cv2.putText(output, "Pan: click and drag", (w-220, h-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1)
    
    # Status
    if analysis_mode:
        if selection_step == 1:
            status_text = "SELECT POINT 1 OF THE SIDELINE"
        elif selection_step == 2:
            status_text = "SELECT POINT 2 OF THE SIDELINE"
        elif selection_step == 3:
            status_text = "SELECT POINT 1 OF THE PENALTY AREA LINE (or 'N' to skip)"
        elif selection_step == 4:
            status_text = "SELECT POINT 2 OF THE PENALTY AREA LINE (or 'N' to skip)"
        elif selection_step == 5:
            status_text = "SELECT POINT 1 OF THE REFERENCE LINE (ANY VISIBLE LINE)"
        elif selection_step == 6:
            status_text = "SELECT POINT 2 OF THE REFERENCE LINE (SAME LINE AS POINT 1)"
        elif selection_step == 7:
            status_text = "SELECT THE DEFENDER (use zoom/pan for greater precision)"
        elif selection_step == 8:
            status_text = "SELECT THE ATTACKER (use zoom/pan for greater precision)"
        else:
        ...
        # Add specific instructions for reference line selection
        if selection_step == 5 or selection_step == 6:
            cv2.putText(output, "MARK PRECISELY ON THE ENDS OF THE MOST VISIBLE LINE AVAILABLE", 
                       (20, h-80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,255), 2)
            cv2.putText(output, "Offside lines will follow EXACTLY this angle", 
                       (20, h-50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,255), 2)
        
        direction_text = f"DIRECTION: {'LEFT' if attacking_direction=='left' else 'RIGHT'}"
        cv2.putText(output, direction_text, (w-300,30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
        
        if calibration_complete:
            cv2.putText(output, "CALIBRATION COMPLETE", (w-300,60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
        elif len(field_points) >= 2:
            cv2.putText(output, "SIDELINE OK", (w-300,60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,0), 2)
            if len(area_points) >= 2:
                cv2.putText(output, "PENALTY AREA LINE OK", (w-300,90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
            if len(goal_line_points) >= 2:
                cv2.putText(output, "REFERENCE LINE OK", (w-300,120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,0,255), 2)
    
    return output

def mouse_click_handler(event, x, y, flags, param):
    """
    Mouse handler for selecting points (sideline, penalty area, goal, defender, attacker)
    and also for pan/zoom.
    """
    global field_points, area_points, goal_line_points
    global defender_point, attacker_point, selection_step
    global zoom_level, pan_x, pan_y, is_panning, pan_start_x, pan_start_y
    ...
    # Correct coordinates for zoom and pan
    original_x = int((x - pan_x)/zoom_level)
    original_y = int((y - pan_y)/zoom_level)
    
    if event == cv2.EVENT_LBUTTONDOWN:
        print(f"Mouse clicked at ({x},{y}) -> Original: ({original_x},{original_y})")
        
        # Check if we are in analysis mode
        if analysis_mode:
            # Point selection flow
            if selection_step == 1:  # Point 1 of sideline
                field_points = [(original_x, original_y)]
                selection_step = 2
                print("Sideline point 1 marked. Mark point 2.")
            elif selection_step == 2:  # Point 2 of sideline
                field_points.append((original_x, original_y))
                selection_step = 3
                print("Sideline point 2 marked. Now mark point 1 of the penalty area.")
            elif selection_step == 3:  # Point 1 of penalty area
                area_points = [(original_x, original_y)]
                selection_step = 4
                print("Penalty area point 1 marked. Mark point 2 of the penalty area.")
            elif selection_step == 4:  # Point 2 of penalty area
                area_points.append((original_x, original_y))
                selection_step = 5
                print("Penalty area point 2 marked. Now mark point 1 of the reference line.")
                # Try calculating preliminary homography
                if len(field_points) >= 2 and len(area_points) >= 2:
                    calculate_homography_from_field_lines()
            elif selection_step == 5:  # Point 1 of reference line
                # IMPORTANT: Here we define point 1 of the reference line
                # This is critical for correct offside line calculation
                goal_line_points = [(original_x, original_y)]
                selection_step = 6
                print("Reference line point 1 marked. Mark point 2 (on the same line).")
                print("IMPORTANT: Mark PRECISE points on the same line - any visible line available.")
                print("TIP: Use the goal line, small area line, large area line, or any other visible line.")
            elif selection_step == 6:  # Point 2 of reference line
                # IMPORTANT: Here we define point 2 of the reference line
                # Both points together define the offside line direction
                goal_line_points.append((original_x, original_y))
                selection_step = 7
                # Recalculate final homography with all points
                calculate_homography_from_field_lines()
                print("Reference line point 2 marked. Now select the defender.")
                print("\n=== ZOOM INSTRUCTIONS ===")
                print("To ZOOM: Use the + and - keys or i and o")
                print("To PAN the image: Click and drag with the mouse")
                print("To RESET zoom and pan: Press Z")
                print("===========================\n")
                # Reset zoom and pan to facilitate initial visualization
                zoom_level = 1.0
                pan_x = 0
                pan_y = 0
                print("You can use zoom (+/- or i/o keys) and pan (drag) to select players with greater precision.")
                print("Use the + and - keys for zoom, or scroll the mouse wheel.")
            elif selection_step == 7:  # Defender
                # In steps 7 and 8, we allow zoom for greater precision
                # It's critical to use original coordinates (without zoom applied)
                defender_point = (original_x, original_y)
                selection_step = 8
                print("Defender marked. Now select the attacker.")
                print(f"Defender original coordinates: {defender_point}")
                print("Continue using zoom/pan if necessary for greater precision.")
            elif selection_step == 8:  # Attacker
                # It's critical to use original coordinates (without zoom applied)
                attacker_point = (original_x, original_y)
                print(f"Attacker original coordinates: {attacker_point}")
                selection_step = 9
                print("Attacker marked. Analysis complete!")
            else:
                # If past step 8, start pan on any click
                is_panning = True
                pan_start_x = x
                pan_start_y = y
        else:
            # If not in analysis mode, start pan
            is_panning = True
            pan_start_x = x
            pan_start_y = y
        ...
        # End pan when button is released
        is_panning = False
    
    elif event == cv2.EVENT_MOUSEMOVE and is_panning:
        # Update pan while mouse moves with button pressed
        pan_x += (x - pan_start_x)
        pan_y += (y - pan_start_y)
        pan_start_x = x
        pan_start_y = y
        
        # Update visualization during pan
        if analysis_frame is not None:
            display_frame = draw_simple_frame(analysis_frame)
            cv2.imshow("VAR Analysis System", display_frame)
            cv2.waitKey(1)
    
    # Support for mouse wheel event (MOUSEWHEEL) on Windows and macOS
    elif event == cv2.EVENT_MOUSEWHEEL:
        # Windows: flags > 0 for zoom in, < 0 for zoom out
        if flags > 0:
            zoom_level = min(zoom_max, zoom_level + zoom_step)
            print(f"Zoom in: {zoom_level:.1f}x")
        else:
        ...
        # Update visualization with new zoom
        if analysis_frame is not None:
            display_frame = draw_simple_frame(analysis_frame)
            cv2.imshow("VAR Analysis System", display_frame)
            cv2.waitKey(1)
            
        print(f"Zoom adjusted to {zoom_level:.1f}x. Use pan (drag) to position the image.")
    
    # Support for trackpad events on macOS
    elif event == 10 or event == cv2.EVENT_MOUSEHWHEEL:  # macOS uses EVENT_MOUSEHWHEEL (10)
        # macOS: negative flags for zoom in, positive for zoom out (opposite of Windows)
        direction = -1 if flags < 0 else 1  # Invert direction on macOS
...
def save_calibration(filename="calibration.json"):
    """Saves calibration settings to JSON."""
    global field_points, area_points, goal_line_points, homography_matrix, calibration_complete, perspective_correction
...
def export_analysis_image(frame, filename=None):
    """Exports the current frame to an image file."""
    if filename is None:
        import time
        filename = f"var_analysis_{int(time.time())}.jpg"
    try:
        cv2.imwrite(filename, frame)
        print(f"Analysis exported as image: {filename}")
        return True
    except Exception as e:
...
def export_analysis_video(cap, start_frame, end_frame, filename=None, duration_seconds=10):
    """Exports a short video of the analysis."""
    global current_frame_num
    if filename is None:
        import time
        filename = f"var_analysis_{int(time.time())}.mp4"
    ...
    # Define line duration (seconds) and calculate number of frames
    line_duration_seconds = 3.0  # Show lines for 3 seconds
    line_duration_frames = int(fps * line_duration_seconds)
    line_start_frame = current_frame_num
    line_end_frame = line_start_frame + line_duration_frames
    ...
    try:
        ...
        for fnum in range(start_frame, end_frame+1):
            ...
            # Check if we are in the period to show lines
            show_lines = fnum >= line_start_frame and fnum <= line_end_frame
            
            # Draw offside lines if available and it's time to show them
            if defender_point and show_lines:
                # Use the same improved offside line calculation
                off_line = calculate_offside_line(defender_point)
                p1_orig, p2_orig = off_line
                
                # Clip to fit screen
                clipped_off = clip_line_to_screen([p1_orig, p2_orig], width, height)
                cv2.line(display_frame, clipped_off[0], clipped_off[1], (255, 255, 255), 5)  # White border
                cv2.line(display_frame, clipped_off[0], clipped_off[1], (0, 0, 255), 3)      # Red line
                
                # Add text "DEFENDER'S LINE"
                mid_x = (clipped_off[0][0] + clipped_off[1][0]) // 2
                mid_y = (clipped_off[0][1] + clipped_off[1][1]) // 2
                cv2.putText(display_frame, "DEFENDER'S LINE", (mid_x-100, mid_y-10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

            if attacker_point and show_lines:
                # Use the same improved attacker's line calculation
                att_line = calculate_attacker_line(attacker_point)
                p1_orig, p2_orig = att_line
                
                # Clip to fit screen
                clipped_att = clip_line_to_screen([p1_orig, p2_orig], width, height)
                cv2.line(display_frame, clipped_att[0], clipped_att[1], (255, 255, 255), 5)  # White border
                cv2.line(display_frame, clipped_att[0], clipped_att[1], (255, 0, 0), 3)      # Blue line
                
                # Add text "ATTACKER'S LINE"
                mid_x = (clipped_att[0][0] + clipped_att[1][0]) // 2
                mid_y = (clipped_att[0][1] + clipped_att[1][1]) // 2 + 30
                cv2.putText(display_frame, "ATTACKER'S LINE", (mid_x-100, mid_y+10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
                ...
                # Display larger and more visible status
                text_size = cv2.getTextSize(status_text, cv2.FONT_HERSHEY_SIMPLEX, 1.5, 3)[0]
                text_x = width // 2 - text_size[0] // 2
                cv2.rectangle(display_frame, (text_x-20, 30), (text_x+text_size[0]+20, 80), (0,0,0), -1)
                cv2.putText(display_frame, status_text, (text_x, 70), 
            ...
            # Draw players with highlight only during line period
            for det in detections:
                box = det[0]
                x1,y1,x2,y2 = [int(v) for v in box]
                foot_point = (int((x1+x2)/2), y2)
                ...
                if show_lines and defender_point and abs(foot_point[0]-defender_point[0])<20 and abs(foot_point[1]-defender_point[1])<20:
                    color = DEFENDER_COLOR
                    cv2.putText(display_frame, "DEFENDER", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                elif show_lines and attacker_point and abs(foot_point[0]-attacker_point[0])<20 and abs(foot_point[1]-attacker_point[1])<20:
                    color = ATTACKER_COLOR
                    cv2.putText(display_frame, "ATTACKER", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                else:
                    # Highlight all players with green rectangles for the rest of the video
                    color = (0,255,0)
                
                cv2.rectangle(display_frame, (x1,y1), (x2,y2), color, 2)
                cv2.circle(display_frame, foot_point, 8, color, -1)
            
            # Add time counter when lines are visible
            if show_lines:
                seconds_remaining = (line_end_frame - fnum) / fps
                if seconds_remaining > 0:
                    cv2.putText(display_frame, f"Analysis: {seconds_remaining:.1f}s", (width-200, height-50),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
            ...
            if (fnum - start_frame)%10==0:
                prog = (fnum - start_frame)/(end_frame - start_frame)*100
                print(f"Exporting video: {prog:.1f}%")
...
def create_control_panel():
    """Example of a control panel (optional)."""
    pass  # If you want to create trackbars, etc.
...
def main():
    ...
    debug_mouse = True  # Enables debug messages for mouse interactions
    ...
    print("\nInstructions:")
    print("S - Start/stop analysis")
    print("Space - Pause/resume video")
    print("Q - Quit")
    print("Select points as per on-screen instructions.\n")
    
    # Show analysis mode status
    if debug_mouse:
        print(f"INITIAL STATUS: analysis_mode={analysis_mode}, selection_step={selection_step}")
    ...
    while running:
        try:
            ...
            if playback_mode:
                if 0 <= playback_index < len(playback_frames):
                    ...
                    if attacker_point:
                        ...
                        is_offside = check_offside(defender_point, attacker_point, attacking_direction)
                        stxt = "OFFSIDE!" if is_offside else "LEGAL POSITION"
                        scol = (0,0,255) if is_offside else (0,255,0)
                        cv2.putText(display_frame, stxt, (width//2-100,50), cv2.FONT_HERSHEY_SIMPLEX,1.0,scol,2)
                    ...
                    frame_info = f"Frame: {playback_index+1}/{len(playback_frames)} (Speed: {playback_speed}x)"
                    cv2.putText(display_frame, frame_info, (20,height-20), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
                    cv2.putText(display_frame, "P: Stop playback | +/-: Speed", (20,30), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
                ...
                if not ret:
                    frame_read_errors += 1
                    print(f"Error reading frame {frame_num} (Error {frame_read_errors}/5)")
                    if frame_read_errors >= 5:
                        print("Too many read errors. Restarting video...")
                        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                        current_frame_num = 0
                        frame_offset = 0
                        frame_read_errors = 0
                ...
                if analysis_mode and analysis_frame is not None:
                    ...
                    for det in detections:
                        ...
                        cv2.rectangle(display_frame, (x1,y1), (x2,y2), (0,255,0), 2)
                    if not analysis_mode:
                        cv2.putText(display_frame, "Press 'S' for analysis", (20,height-20),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255),2)
                ...
                if paused:
                    ...
                    frame_text += f" (Speed: {playback_speed_normal:.1f}x)"
                cv2.putText(display_frame, frame_text, (width-400, height-20),
                            cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,255,255),2)
                
                # Add analysis mode and selection status for debugging
                if debug_mouse and analysis_mode:
                    debug_text = f"analysis_mode: {analysis_mode}, step: {selection_step}"
                    cv2.putText(display_frame, debug_text, (20, height-50), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
            ...
            if key == ord(' '):
                paused = not paused
                print("Video paused" if paused else "Video continuing")
            elif key in [ord('s'), ord('S')]:
                if not analysis_mode:
                    paused = True
                    analysis_mode = True
                        ...
                        if ret:
                            ...
                            if len(results) > 0 and hasattr(results[0], 'boxes'):
                                ...
                                for i in range(len(boxes)):
                                    ...
                                    cls = boxes.cls[i].cpu().numpy()
                                    analysis_detections.append((box, conf, cls))
                            print(f"{len(analysis_detections)} detections in the analysis frame.")
                            print("SELECT POINT 1 OF THE SIDELINE (click on the image)")
                            display_frame = draw_simple_frame(analysis_frame)
                            cv2.imshow(window_name, display_frame)
                            cv2.waitKey(1)
                        else:
                            print("ERROR: Could not capture frame for analysis.")
                            analysis_mode = False
                    except Exception as e:
                ...
                # Add 'N' key to skip the current calibration step
                if 3 <= selection_step <= 6:
                    if selection_step == 3 or selection_step == 4:
                        # Skip penalty area points
                        if selection_step == 3:
                            print("Skipping penalty area selection.")
                            area_points = []
                        selection_step = 5
                        print("Now select point 1 of the goal line.")
                    elif selection_step == 5 or selection_step == 6:
                        # Skip goal line points
                        if selection_step == 5:
                            print("Skipping goal line selection.")
                            goal_line_points = []
                        selection_step = 7
                        print("Now select the defender.")
                ...
                attacking_direction = 'left'
                print("Attack direction set to LEFT.")
                if analysis_frame is not None:
                    df = draw_simple_frame(analysis_frame)
                    cv2.imshow(window_name, df)
            elif key == ord('d'):
                attacking_direction = 'right'
                print("Attack direction set to RIGHT.")
                if analysis_frame is not None:
                    df = draw_simple_frame(analysis_frame)
                    cv2.imshow(window_name, df)
            elif key == ord('r'):
                if analysis_mode:
                    ...
                    pan_x = 0
                    pan_y = 0
                    print("Analysis reset. Select sideline.")
                    if debug_mouse:
                        print(f"RESET: analysis_mode={analysis_mode}, selection_step={selection_step}")
                    if analysis_frame is not None:
                        df = draw_simple_frame(analysis_frame)
                    ...
                    if not playback_mode:
                        ...
                        for f in range(startf, endf + 1):
                            ...
                            if ret:
                                ...
                                playback_frames.append((frm.copy(), dets))
                        cap.set(cv2.CAP_PROP_POS_FRAMES, current_pos)
                        print(f"{len(playback_frames)} frames captured for playback.")
                    else:
                        playback_mode = False
                        print("Playback mode stopped.")
            elif key in [ord('+'), ord('=')]:
                if analysis_mode and (selection_step == 7 or selection_step == 8):
                    # Zoom in during defender/attacker selection - PRIORITY
                    old_zoom = zoom_level
                    zoom_level = min(zoom_max, zoom_level + zoom_step)
                    print(f"Zoom in: {zoom_level:.1f}x")
                    ...
                    print(f"NOTE: Original coordinates are preserved with zoom={zoom_level:.1f}x")
                    if defender_point:
                        print(f"Defender at: {defender_point}")
                    if attacker_point:
                        print(f"Attacker at: {attacker_point}")
                    ...
                    playback_speed_normal = min(5.0, playback_speed_normal + 0.5)
                    print(f"Speed: {playback_speed_normal:.1f}x")
            elif key in [ord('-'), ord('_')]:
                if analysis_mode and (selection_step == 7 or selection_step == 8):
                    # Zoom out during defender/attacker selection - PRIORITY
                    old_zoom = zoom_level
                    zoom_level = max(zoom_min, zoom_level - zoom_step)
                    print(f"Zoom out: {zoom_level:.1f}x")
                    ...
                    playback_speed_normal = max(0.5, playback_speed_normal - 0.5)
                    print(f"Speed: {playback_speed_normal:.1f}x")
            # Add alternative zoom keys: 'i' for zoom in and 'o' for zoom out
            elif key == ord('i') and analysis_mode and (selection_step == 7 or selection_step == 8):
                # 'i' for zoom in (alternative to +)
                old_zoom = zoom_level
                zoom_level = min(zoom_max, zoom_level + zoom_step)
                print(f"Zoom in (i): {zoom_level:.1f}x")
                ...
                use_auto_vanishing = not use_auto_vanishing
                print("AUTO vanishing point" if use_auto_vanishing else "MANUAL vanishing point")
                if analysis_mode and analysis_frame is not None and defender_point:
                    df = draw_simple_frame(analysis_frame)
                    cv2.imshow(window_name, df)
            elif key == ord('c'):
                ...
                # 'V' key to export analysis video
                if analysis_mode and defender_point is not None:
                    export_analysis_video(cap, None, None)
            elif key == ord('e'):
                # 'E' key to export analysis image
                if analysis_mode and analysis_frame is not None:
                    frame_to_save = draw_simple_frame(analysis_frame)
                    export_analysis_image(frame_to_save)
            elif key == ord('d') and debug_mouse:
