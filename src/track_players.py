...
# Cache to store the tracker between calls
global_tracker = None

def track_players(frame, detections, tracker=None):
    """
    Tracks players in a frame using DeepSort.
    
    Args:
        frame: The current frame.
        detections: List of detections in the format [(bbox, score, class_id), ...].
        tracker: The existing tracker object for reuse (optional).
    
    Returns:
        A TrackPlayers object with the tracked players.
    """
    # Initialize the tracker if not provided
    if tracker is None:
        tracker = DeepSort(
            max_age=30,
            n_init=3,
    ...
    # Format the detections for DeepSort
    formatted_detections = []
    for det in detections:
        bbox = det[0]  # [x1, y1, x2, y2]
        score = det[1]
        ...
        # Convert to the format expected by DeepSort: [x, y, w, h, confidence]
        x1, y1, x2, y2 = bbox
        w = x2 - x1
        h = y2 - y1
        formatted_detections.append(([x1, y1, w, h], score, class_id))
...
class TrackPlayers:
    ...
    def get_track_positions(self):
        """Returns the positions (centers) of all tracks."""
        positions = []
        for track in self.tracks:
            if hasattr(track, 'to_tlwh'):
                bbox = track.to_tlwh()
                x, y, w, h = bbox
                center_x = x + w/2
                foot_y = y + h  # Foot position (bottom part)
                positions.append((center_x, foot_y))
        return positions
    
    def get_track_boxes(self):
        """Returns the bounding boxes of all tracks."""
        boxes = []
        for track in self.tracks:
            if hasattr(track, 'to_tlbr'):
                bbox = track.to_tlbr()  # [x1, y1, x2, y2]
