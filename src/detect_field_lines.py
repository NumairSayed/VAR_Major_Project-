...
# Cache to store previous field lines and reduce fluctuations
_previous_lines = None
_stability_counter = 0

def detect_field_lines(frame):
    """
    Detects field lines in a soccer video frame with inter-frame stability.
    ...
    # Detect edges with stricter parameters
    edges = cv2.Canny(enhanced, 70, 200)
    ...
    if lines is not None:
        for line in lines:
            ...
            # Filter lines (keep only near-horizontal/vertical with significant length)
            if length > 80 and (abs(angle) < 15 or abs(angle) > 165 or  # near horizontal
                abs(angle - 90) < 15 or abs(angle + 90) < 15):  # near vertical
                field_lines.append((x1, y1, x2, y2))
    
    # If no lines detected, use previous ones if available
    if not field_lines and _previous_lines is not None:
        _stability_counter += 1
        if _stability_counter < 10:  # Use previous lines for up to 10 frames
            return _previous_lines
        else:
            _stability_counter = 0
            return []
    
    # Limit number of lines to avoid excessive processing
    if len(field_lines) > 8:
        # Sort by length (longest first)
        field_lines.sort(key=lambda line: np.sqrt((line[2] - line[0])**2 + (line[3] - line[1])**2), reverse=True)
        field_lines = field_lines[:8]
    
    # Stabilization: If we have previous lines, smooth transitions
    if _previous_lines is not None and field_lines:
        _stability_counter = 0
        stabilized_lines = []
        
        # For each current line, find closest previous match and average
        for line in field_lines:
            x1, y1, x2, y2 = line
            
            # If similar previous line exists, average positions
            found_match = False
            for prev_line in _previous_lines:
                px1, py1, px2, py2 = prev_line
                
                # Average distance between line points
                dist = (abs(x1 - px1) + abs(y1 - py1) + abs(x2 - px2) + abs(y2 - py2)) / 4
                
                if dist < 30:  # Threshold to consider same line
